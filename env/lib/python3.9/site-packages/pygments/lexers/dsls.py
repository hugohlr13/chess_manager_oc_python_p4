...
class CrmshLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            # acl_mod:blah
            (r'(%s)(:)("(?:""|[^"])*"|\S+)' % acl_mod, bygroups(Keyword, Punctuation, Name)),
            # rsc_id[:(role|action)]
            # NB: this matches all other identifiers
            (r"([\w#$-]+)(?:(:)(%s))?(?![\w#$-])" % rsc_role_action, bygroups(Name, Punctuation, Operator.Word)),
            # punctuation
            (r"(\\(?=\n)|[\[\](){}/:@])", Punctuation),
            (r"\s+|\n", Whitespace),
        ],
...
class SnowballLexer(ExtendedRegexLexer):
    ...
    tokens = {
        ...
        "root1": [
            ...
            (r"[!*+\-/<=>]=|[-=]>|<[+-]|[$*+\-/<=>?\[\]]", Operator),
            (
                words(("as", "get", "hex", "among", "define", "decimal", "backwardmode"), suffix=r"\b"),
                Keyword.Reserved,
            ),
            (
            ...
            (r"(stringdef\b)([%s]*)([^%s]+)" % (_ws, _ws), bygroups(Keyword.Reserved, Whitespace, String.Escape)),
