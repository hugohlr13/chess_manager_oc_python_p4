...
class PropertiesLexer(RegexLexer):
    ...
    tokens = {
        ...
        "separator": [
            ...
            (r"([ \f\t]*)([=:]*)([ \f\t]*)(.*)", bygroups(Whitespace, Operator, Whitespace, String), "#pop"),
        ],
        "value": [  # line continuation
            (r"\s+", Whitespace),
            # search for line continuation escape
            (r"(\s*)(.*(?<!\\)(?:\\{2})*)(\\)(?!\\)([ \t]*)", bygroups(Whitespace, String, Text, Whitespace)),
            (r".*$", String, "#pop"),
        ],
    }
...
class DockerLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(HEALTHCHECK)(\s+)((%s--\w+=\w+%s)*)" % (_lb, _lb), bygroups(Keyword, Whitespace, using(BashLexer))),
            (
                r"(VOLUME|ENTRYPOINT|CMD|SHELL)(\s+)(%s)(\[.*?\])" % (_lb,),
                bygroups(Keyword, Whitespace, using(BashLexer), using(JsonLexer)),
            ),
            (r"(LABEL|ENV|ARG)(\s+)((%s\w+=\w+%s)*)" % (_lb, _lb), bygroups(Keyword, Whitespace, using(BashLexer))),
