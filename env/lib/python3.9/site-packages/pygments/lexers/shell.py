...
class BatchLexer(RegexLexer):
    ...
    _keyword_terminator = r"(?=(?:\^[%s]?)?[%s+./:[\\\]]|[%s%s(])" % (_nl, _ws, _nl, _punct)
    ...
    def _make_begin_state(
        ...
        return [
            (
                (r"\)", Punctuation, "#pop")
                if compound
                else (r"\)((?=\()|%s)%s" % (_token_terminator, rest_of_line), Comment.Single)
            ),
            ...
            (
                r"((?:for|if|rem)(?:(?=(?:\^[%s]?)?/)|(?:(?!\^)|"
                r"(?<=m))(?:(?=\()|%s)))(%s?%s?(?:\^[%s]?)?/(?:\^[%s]?)?\?)"
                % (_nl, _token_terminator, _space, _core_token_compound if compound else _core_token, _nl, _nl),
                bygroups(Keyword, using(this, state="text")),
                "follow%s" % suffix,
            ),
            (
                r"(goto%s)(%s(?:\^[%s]?)?/(?:\^[%s]?)?\?%s)" % (_keyword_terminator, rest, _nl, _nl, rest),
                bygroups(Keyword, using(this, state="text")),
                "follow%s" % suffix,
            ),
            (
            ...
            (r"(call)(%s?)(:)" % _space, bygroups(Keyword, using(this, state="text"), Punctuation), "call%s" % suffix),
            (r"call%s" % _keyword_terminator, Keyword),
            (
                r"(for%s(?!\^))(%s)(/f%s)" % (_token_terminator, _space, _token_terminator),
                bygroups(Keyword, using(this, state="text"), Keyword),
                ("for/f", "for"),
            ),
            (
                r"(for%s(?!\^))(%s)(/l%s)" % (_token_terminator, _space, _token_terminator),
                bygroups(Keyword, using(this, state="text"), Keyword),
                ("for/l", "for"),
            ),
            ...
            (
                ...
                % (_token_terminator, _space, _token_terminator, _space, _token_terminator, _space),
                bygroups(
                    Keyword,
    ...
    def _make_follow_state(
        ...
        state += [
            (
                r"%s([%s]*)(%s)(.*)" % (_start_label, _ws, _label_compound if compound else _label),
    ...
    def _make_redirect_state(
        ...
        stoken_compound = r"(?:[%s]+|(?:%s|%s|%s)+)" % (_punct, _string, _variable, _core_token_compound)
        return [
            (
    ...
    tokens = {
        ...
        "variable-or-escape": [(_variable, Name.Variable), (r"%%%%|\^[%s]?(\^!|[\w\W])" % _nl, String.Escape)],
        "string": [
            (r'"', String.Double, "#pop"),
        ...
        "for2": [
            (r"\)", Punctuation),
            (r"(%s)(do%s)" % (_space, _token_terminator), bygroups(using(this, state="text"), Keyword), "#pop"),
            (r"[%s]+" % _nl, Text),
            include("follow"),
        ],
        "for/f": [
        ...
        "if": [
            (
                r"((?:cmdextversion|errorlevel)%s)(%s)(\d+)" % (_token_terminator, _space),
...
class TcshLexer(RegexLexer):
    ...
    tokens = {
        ...
        "basic": [
            (
                r"\b(if|endif|else|while|then|foreach|case|default|" r"continue|goto|breaksw|end|switch|endsw)\s*\b",
                Keyword,
            ),
            (
...
class PowerShellLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"\(", Punctuation, "child"),
            (r"\s+", Text),
            (r"^(\s*#[#\s]*)(\.(?:%s))([^\n]*$)" % "|".join(commenthelp), bygroups(Comment, String.Doc, Comment)),
            (r"#[^\n]*?$", Comment),
