...
class Inform6Lexer(RegexLexer):
    ...
    tokens = {
        "root": [(r"\A(!%%[^%s]*[%s])+" % (_newline, _newline), Comment.Preproc, "directive"), default("directive")],
        ...
        "expression": [
            ...
            (r"&&?|\|\|?|[=~><]?=|[%s]{1,2}>?|\.\.?[&#]?|::|[<>+*/%%]" % _dash, Operator, "_expression"),
        ...
        "value": [
            ...
            (r"([%s])(@.{2})([%s])" % (_squote, _squote), bygroups(String.Char, String.Escape, String.Char), "#pop"),
            (r"[%s]" % _squote, String.Single, ("#pop", "dictionary-word")),
            (r"[%s]" % _dquote, String.Double, ("#pop", "string")),
            # Numbers
            (r"\$[<>]?[+%s][0-9]*\.?[0-9]*([eE][+%s]?[0-9]+)?" % (_dash, _dash), Number.Float, "#pop"),
        ...
        "string": [
            ...
            (r"[({]", String.Double),
            (r"\\", String.Escape),
            (r"@(\\\s*[%s]\s*)*@((\\\s*[%s]\s*)*[0-9])*" % (_newline, _newline), String.Escape),
            (
                r"@(\\\s*[%s]\s*)*[({]((\\\s*[%s]\s*)*[0-9a-zA-Z_])*"
                r"(\\\s*[%s]\s*)*[)}]" % (_newline, _newline, _newline),
                String.Escape,
            ),
            (r"@(\\\s*[%s]\s*)*.(\\\s*[%s]\s*)*." % (_newline, _newline), String.Escape),
    ...
    for level in token_variants:
        tokens[level] = {
            ...
            "+i6t-root": [  # For Inform6TemplateLexer
                (r"[^%s]*" % Inform6Lexer._newline, Comment.Preproc, ("directive", "+p"))
            ],
            "root": [
                (r"(\|?\s)+", Text),
                (r"\[", Comment.Multiline, "+comment"),
                (r"[%s]" % _dquote, Generic.Heading, ("+main", "+titling", "+titling-string")),
                default(("+main", "+heading?")),
            ],
            "+titling-string": [(r"[^%s]+" % _dquote, Generic.Heading), (r"[%s]" % _dquote, Generic.Heading, "#pop")],
            "+titling": [
                (r"\[", Comment.Multiline, "+comment"),
                (r"[^%s.;:|%s]+" % (_dquote, _newline), Generic.Heading),
                (r"[%s]" % _dquote, Generic.Heading, "+titling-string"),
                (r"[%s]{2}|(?<=[\s%s])\|[\s%s]" % (_newline, _dquote, _dquote), Text, ("#pop", "+heading?")),
            ...
            "+main": [
                ...
                (r"(%s|(?<=[\s;:.%s]))\|\s|[%s]{2,}" % (_start, _dquote, _newline), Text, "+heading?"),
            ...
            "+heading?": [
                ...
                (r"[%s]{1,3}" % _dash, Text),
                (r"(?i)(volume|book|part|chapter|section)\b[^%s]*" % _newline, Generic.Heading, "#pop"),
                default("#pop"),
            ],
            ...
            "+documentation": [
                (r"(?i)(%s)\s*(chapter|example)\s*:[^%s]*" % (_start, _newline), Generic.Heading),
                (r"(?i)(%s)\s*section\s*:[^%s]*" % (_start, _newline), Generic.Subheading),
            ...
            "+i6t-not-inline": [
                ...
                (r"(%s)@([%s]+|Purpose:)[^%s]*" % (_start, _dash, _newline), Comment.Preproc),
            ...
            "+i6t": [
                (r"(\{[%s])(![^}]*)(\}?)" % _dash, bygroups(Punctuation, Comment.Single, Punctuation)),
                (
                    r"(\{[%s])(lines)(:)([^}]*)(\}?)" % _dash,
            ...
            "+p": [
                (r"[^@]+", Comment.Preproc),
                (r"(%s)@c( .*?)?([%s]|\Z)" % (_start, _newline), Comment.Preproc, "#pop"),
            ...
            "+lines": [
                ...
                (r"(\{)([%s]endlines)(\})" % _dash, bygroups(Punctuation, Keyword, Punctuation), "#pop"),
                (r"[^@!{]+?([%s]|\Z)|." % _newline, Text),
            ],
        }
...
class Tads3Lexer(RegexLexer):
    ...
    def _make_string_state(triple, double, verbatim=None, _escape=_escape):
        ...
        state = []
        if triple:
            state += [(r"%s{3,}" % char, token, "#pop"), (r"\\%s+" % char, String.Escape), (char, token)]
        else:
        ...
        if verbatim:
            ...
            state += [
                (
                    r"\\?<!([^><\\%s]|<(?!<)|\\%s%s|%s|\\.)*>?" % (char, char, escaped_quotes, _escape),
                    Comment.Multiline,
                ),
                (r"(?i)\\?<listing(?=[\s=>]|\\>)", Name.Tag, ("#pop", "%sqs/listing" % prefix, tag_state_name)),
                (r"(?i)\\?<xmp(?=[\s=>]|\\>)", Name.Tag, ("#pop", "%sqs/xmp" % prefix, tag_state_name)),
                (
                    r"\\?<([^\s=><\\%s]|<(?!<)|\\%s%s|%s|\\.)*" % (char, char, escaped_quotes, _escape),
                    Name.Tag,
                    tag_state_name,
                ),
                include("s/entity"),
        ...
        state += [
            include("s/escape"),
            (r"\{([^}<\\%s]|<(?!<)|\\%s%s|%s|\\.)*\}" % (char, char, escaped_quotes, _escape), String.Interpol),
            (r"[\\&{}<]", token),
        ]
        return state
    ...
    def _make_tag_state(triple, double, _escape=_escape):
        ...
        return [
            ...
            (r"\\?>", Name.Tag, "#pop"),
            (r"\{([^}<\\%s]|<(?!<)|\\%s%s|%s|\\.)*\}" % (char, char, escaped_quotes, _escape), String.Interpol),
            (r"([^\s=><\\%s]|<(?!<)|\\%s%s|%s|\\.)+" % (char, char, escaped_quotes, _escape), Name.Attribute),
            include("s/escape"),
            include("s/verbatim"),
            include("s/entity"),
            (r"[\\{}&]", Name.Attribute),
    ...
    def _make_attribute_value_state(terminator, host_triple, host_double, _escape=_escape):
        ...
        escaped_quotes = r"+|%s(?!%s{2})" % (host_char, host_char) if host_triple else r""
        return [
            (r"%s%s" % (host_char, host_quantifier), host_token, "#pop:3"),
            (r"%s%s" % (r"" if token is String.Other else r"\\?", terminator), token, "#pop"),
            include("s/verbatim"),
            include("s/entity"),
            (
                r"\{([^}<\\%s]|<(?!<)|\\%s%s|%s|\\.)*\}" % (host_char, host_char, escaped_quotes, _escape),
                String.Interpol,
            ),
    ...
    tokens = {
        ...
        "block/basic": [
            ...
            (r"(%s)(%s*)(:)" % (_name, _ws), bygroups(Name.Label, using(this, state="whitespace"), Punctuation)),
            include("whitespace"),
        ],
        ...
        "more/basic": [
            ...
            (r"(is|not)(%s+)(in\b)" % _ws, bygroups(Operator.Word, using(this, state="whitespace"), Operator.Word)),
            (r'[^\s!"%-_a-z{-~]+', Error),  # Averts an infinite loop
        ],
        ...
        # Parameter list
        "main/parameters": [
            (r"(%s)(%s*)(?=:)" % (_name, _ws), bygroups(Name.Variable, using(this, state="whitespace")), "#pop"),
            (
        ...
        "more/parameters": [
            (r"(:)(%s*(?=[?=,:)]))" % _ws, bygroups(Punctuation, using(this, state="whitespace"))),
            (r"[?\]]+", Punctuation),
        ...
        "grammar-rules": [
            include("string"),
            include("whitespace"),
            (r"(\[)(%s*)(badness)" % _ws, bygroups(Punctuation, using(this, state="whitespace"), Keyword), "main"),
        ...
        "whitespace": [
            (r"^%s*#(%s|[^\n]|(?<=\\)\n)*\n?" % (_ws_pp, _comment_multiline), Comment.Preproc),
