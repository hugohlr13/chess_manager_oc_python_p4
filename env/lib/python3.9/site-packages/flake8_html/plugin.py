...
jinja2_env = Environment(loader=PackageLoader("flake8_html"))
...
IndexEntry = namedtuple("IndexEntry", "filename report_name error_count highest_sev")
...
class HTMLPlugin(base.BaseFormatter):
    ...
    def finished(self, filename):
        ...
        for code, errors in self.by_code.items():
            ...
            errs = sorted((line, text, count) for (line, text), count in errcounts.items())
            index.append(
                (
                    sev,
                    len(errors),
        ...
        if self.pep8report:
            ...
            pep8_report_errs.sort(key=lambda err: (err.filename, err.line_number, err.column_number))
        ...
        for sev, count in sorted(counts.items()):
            scores.append("%s: %d" % (SEVERITY_NAMES[sev - 1], count))
        if not self.pep8report:
    ...
    def get_report_filename(self, filename, suffix=""):
        ...
        rfname = "{}{}.html".format(stem.replace(os.sep, ".").strip("."), suffix)
    ...
    def _format_source(self, source):
        ...
        return {"html_lines": [Markup(ln) for ln in html.splitlines()], "css": formatter.get_style_defs()}
    ...
    def write_styles(self):
        ...
        rendered = tmpl.render(pygments_css=formatter.get_style_defs())
    ...
    def write_index(self):
        ...
        rendered = jinja2_env.get_template("index.html").render(
            ...
            index=sorted(self.files, key=lambda e: (e.highest_sev, -e.error_count)),
    ...
    def add_options(cls, options):
        ...
        options.add_option("--htmldir", help="Directory in which to write HTML output.", parse_from_config=True)
        options.add_option(
            "--htmltitle",
