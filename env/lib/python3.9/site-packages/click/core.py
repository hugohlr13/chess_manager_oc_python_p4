...
def _check_multicommand(base_command: "MultiCommand", cmd_name: str, cmd: "Command", register: bool = False) -> None:
    ...
    if register:
        ...
        hint = (
            "Found a multi command as subcommand to a multi command" " that is in chain mode. This is not supported."
        )
    raise RuntimeError(
...
class Parameter:
    ...
    def type_cast_value(self, ctx: Context, value: t.Any) -> t.Any:
        ...
        if self.nargs == 1 or self.type.is_composite:
            convert: t.Callable[[t.Any], t.Any] = partial(self.type, param=self, ctx=ctx)
        elif self.nargs == -1:
