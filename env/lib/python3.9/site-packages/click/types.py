    ...
    def convert(self, value: t.Any, param: t.Optional["Parameter"], ctx: t.Optional["Context"]) -> t.Any:
        try:
            ...
            if lazy:
                f: t.IO = t.cast(
                    t.IO,
                    LazyFile(value, self.mode, self.encoding, self.errors, atomic=self.atomic),
                )
    ...
    def convert(self, value: t.Any, param: t.Optional["Parameter"], ctx: t.Optional["Context"]) -> t.Any:
        ...
        if not is_dash:
            ...
            if not self.file_okay and stat.S_ISREG(st.st_mode):
                self.fail(
                    _("{name} {filename!r} is a file.").format(name=self.name.title(), filename=os.fsdecode(value)),
                    param,
                    ctx,
                )
