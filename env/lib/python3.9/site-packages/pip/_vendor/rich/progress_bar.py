...
class ProgressBar(JupyterMixin):
    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        ...
        complete_halves = int(width * 2 * completed / self.total) if self.total else width * 2
        bar_count = complete_halves // 2
        half_bar_count = complete_halves % 2
        style = console.get_style(self.style)
        complete_style = console.get_style(self.complete_style if self.completed < self.total else self.finished_style)
        _Segment = Segment
        if bar_count:
